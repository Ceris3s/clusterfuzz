...
def _set_public_acl_if_needed(url):
  ...
  if storage.get_acl(url, 'allUsers'):
    logs.log(f'{url} is already marked public, skipping.')
    return True

  if not storage.set_acl(url, 'allUsers'):
    logs.log_error(f'Failed to mark {url} public.')
    return False
...
def _make_corpus_backup_public(target, corpus_fuzzer_name_override,
  ...
  if not storage.get(corpus_backup_url):
    logs.log_warn(f'Failed to find corpus backup {corpus_backup_url}.')
    return
  ...
  if not storage.copy_blob(corpus_backup_url, public_url):
    logs.log_error(
        f'Failed to overwrite {public_url} with the latest public corpus backup.'
    )
    return
  ...
  logs.log(f'Corpus backup {corpus_backup_url} is now marked public.')
